...
export class UserNames {
  ...
  getRandomName = ({ id }: Socket) => {
    const index1 = Math.floor(Math.random() * this.#randomP.length);
    const index2 = Math.floor(Math.random() * this.#randomAnimals.length);
    const randomName = `${this.#randomP[index1]} ${this.#randomAnimals[index2]}`;
